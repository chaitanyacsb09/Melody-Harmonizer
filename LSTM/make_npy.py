...
def make_test_npys(file_name, song_sequence, song_chords):
    ...
    # exit()
    np.save('%s/%s.npy' % (file_path, file_name.split('.')[0]), np.array(combined, dtype=object))
...
def main():
    ...
    for csv_path in csv_files:
        ...
        if _input == '2':
            ...
            make_test_npys(ntpath.basename(csv_path), song_sequence, song_Chords)

    if _input == '1':
        np.save('dataset/input_vector.npy', np.array(result_input_matrix, dtype=object))
        np.save('dataset/target_vector.npy', np.array(result_target_matrix))
    elif _input == '2':
        np.save('dataset/test_vector.npy', np.array(result_input_matrix, dtype=object))
    
    # for vec in result_input_matrix:
